---
title: "Live Handson Praktikum Anreg ke-4"
...
```{r}
set.seed(2026) #Mengunci angka acak agar hasil simulasi tetap sama setiap kali dijalankan.
n <- 100
p <- 3
...
# eksplorasi Data
Bagian ini bertujuan untuk melihat hubungan awal antara variabel $x$ dan $y$.

```{r}
par(mfrow=c(1,2)) #Membagi jendela grafik menjadi 1 baris dan 2 kolom.
plot(x1, y, pch=19) #Buat Scatter Plot
plot(x2, y, pch=19) #Buat Scatter Plot
```
...
```{r}
sigma_kuadrat <- (t(y)%*%y-t(b)%*%t(X)%*%y)/(n-p) #Menghitung varians dari galat (Residual Mean Square).
...
```
```{r}
y_duga <- b0+b1*x1+b2*x2 #Menghitung nilai $y$ prediksi berdasarkan parameter yang ditemukan
Y <- data.frame(y,y_duga);Y
```

```{r}
R_squared <- (cor(y,y_duga))^2;round(R_squared,4) #Mengukur seberapa besar model mampu menjelaskan variasi data.
```

```{r}
R_squared_adj <- 1-((1-R_squared)*(n-1)/(n-p));round(R_squared_adj,4) #Penyesuaian $R^2$ agar lebih objektif karena adanya lebih dari satu variabel independen.
```

## Uji F dan Standar Error Parameter
Uji F igunakan untuk menguji apakah model secara keseluruhan (semua $x$) berpengaruh terhadap $y$.
